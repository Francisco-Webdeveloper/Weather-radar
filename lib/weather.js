...
// reads the user's input to request the weather condition of the chosen city
const addweather = (event) => {
  event.preventDefault(); // prevents the browser reloading behavior
  const input = document.getElementById("city").value;
  const results = document.querySelector("#weather");
  // adding white background
  results.classList.add("white-bg");
  fetchApiWeatherByCity(input);
};

export { addweather, updateCard };
